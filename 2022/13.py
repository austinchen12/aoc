...
[[0,2,10],[[],3],[[[0,2,6,5],[5,4,4]],[],[],[],4],[[1,[2],8,[7,8,0,3,7]],7,7,1,[2,[8,3,1],[],[7,8,2,7]]]]""".split('\n\n')

#####
def cmp(left, right):
    if type(left) == int and type(right) == int:
        return 'TIE' if left == right else left < right
    ...
    return 'TIE' if len(left) == len(right) else len(left) < len(right)
def parse_array(line, i=0):
    result = []
    assert line[i] == '['
    i += 1
    while i < len(line):
        if line[i] == '[':
            temp, i = parse_array(line, i)
            result.append(temp)
        elif line[i].isdigit():
            digits = []
...
for i, chunk in enumerate(data):
    ...
    l, r = parse_array(left)[0], parse_array(right)[0]
    if cmp(l, r):
        count += i + 1
print(count)
...
def cmp(left, right):
    ...
    if len(left) == len(right):
        ...
        return 1
dividers = [[2]], [[6]]
packets = [tuple(dividers[0]), tuple(dividers[1])]
for chunk in data:
    left, right = chunk.split('\n')
    packets.append(tuple(parse_array(left)[0]))
    packets.append(tuple(parse_array(right)[0]))
packets.sort(key=cmp_to_key(cmp))
print((packets.index(tuple(dividers[0])) + 1) * (packets.index(tuple(dividers[1])) + 1))