...
def capture_number(data, sr, sc):
    # Capture all digits to the left, and all to the right
    left = []
    left_i = sc - 1
    while 0 <= left_i < len(data[0]) and data[sr][left_i].isdigit():
        left.append(data[sr][left_i])
        left_i -= 1
    
    right = []
    right_i = sc + 1
    while 0 <= right_i < len(data[0]) and data[sr][right_i].isdigit():
        right.append(data[sr][right_i])
        right_i += 1
    return int(''.join(reversed(left)) + data[sr][sc] + ''.join(right)), ((sr, left_i - 1), (sr, right_i - 1))
...
for r in range(len(data)):
    for c in range(len(data[r])):
        ...
        # Symbol, check each direction. capture entire number, keep track in seen to make sure no duplicates
        for dr, dc in DIRS:
            if 0 <= r + dr < len(data) and 0 <= c + dc < len(data[0]) and data[r + dr][c + dc].isdigit(): # check in bounds and is digit
                number, num_range = capture_number(data, r + dr, c + dc)
                if not num_range in seen:
                    sum_ += number
                    seen.add(num_range)
print(sum_)
