data = """Game 1: 4 green, 2 blue; 1 red, 1 blue, 4 green; 3 green, 4 blue, 1 red; 7 green, 2 blue, 4 red; 3 red, 7 green; 3 red, 3 green
...
Game 100: 4 blue, 14 red; 12 red, 1 blue; 2 red, 2 blue; 8 red; 14 red, 2 blue, 1 green; 3 blue""".split('\n')
...
ids = 0
regex = re.compile(r'Game (\d+): (.+)')
for line in data:
    id, game = regex.match(line).groups()
    draws = game.split('; ')
    if check_game(draws):
        ids += int(id)
print(ids)
...
def check_game(draws):
    ...
    for draw in draws:
        ...
        for group in groups:
            ...
            max_counts[color] = max(max_counts[color], number)
    return max_counts
