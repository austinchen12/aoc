...
data = """-F|..FF.F77|7-777FLF7F|7-F--F--FF7F7F.FL-|-|777F-7FL7F77---|-7-77.F-LLF7FF|-F-7.FF77F|7---F7.7FF7-J7-FF.F77F--.-|.F.F7L7--J.FLL7J7L|J7L|.F77
...
|-|-J.--LLLF|JJ.FFLJL-J-JJL--JF-L7-|J.F.LJ.L-J|L|77--J|-J.JL|.J.LJJL-JJLJJ.-J-J-7-7-LJL7L|-L-JJLLFJF--L|J--JFJF-7-LLJL--|---F.--|7J.L-L7JJ-.""".split('\n')
...
STARTING_CONNECTIONS = ['-LF', '|7F', '-J7', '|LJ']
...
for r in range(len(data)):
    for c in range(len(data[r])):
        if data[r][c] == 'S':
            start = (r, c)
visited, queue, depth = set(), [[start]], -1
while queue:
    level = queue.pop()
    new = []
    for r, c in level:
        if (r, c) in visited or not (0 <= r < len(data) and 0 <= c < len(data[0])):
             continue
        ...
        if (r, c) == start:
            for i in range(len(DIRS)):
                dr, dc = DIRS[i]
                if 0 <= r + dr < len(data) and 0 <= c + dc < len(data[0]) and data[r + dr][c + dc] in STARTING_CONNECTIONS[i]:
    ...
    for c in range(len(data[0])):
        if data[r][c] == 'S':
            ...
            for i in range(len(DIRS)):
                dr, dc = DIRS[i]
                if 0 <= r + dr < len(data) and 0 <= c + dc < len(data[0]) and data[r + dr][c + dc] in STARTING_CONNECTIONS[i]:
                    possible_tile += STARTING_CONNECTIONS[(i + 2) % 4]
            counts = { possible_tile.count(c): c for c in possible_tile }
            data[r][c] = counts[2]
def check_direction(data, start, direction):
    r, c = start
    count, open = 0, None
    if direction in 'LR':
        rng = range(0, c) if direction == 'L' else range(c + 1, len(data[0]))
        for i in rng:
            ...
            if data[r][i] in '|':
                ...
                open = data[r][i]
            elif data[r][i] in '7J' and open:
                count += 2 if (open + data[r][i] == 'LJ' or open + data[r][i] == 'F7') else 1
                open = None
    else:
        rng = range(0, r) if direction == 'U' else range(r + 1, len(data))
        for i in rng:
            ...
            if data[i][c] in '-':
                ...
                open = data[i][c]
            elif data[i][c] in 'LJ' and open:
                assert open != None
                count += 2 if (open + data[i][c] == '7J' or open + data[i][c] == 'FL') else 1
                open = None
    return count % 2 == 1
count = 0
clone = copy.deepcopy(data)
# print('\n'.join([''.join(line) for line in data]))
# print(check_direction(data, (4, 13), 'D'))
for r in range(len(data)):
    for c in range(len(data[0])):
        if (r, c) not in visited:
            for dir in ['L', 'U', 'R', 'D']:
                if not check_direction(data, (r, c), dir):
                    break
            else:
                clone[r][c] = 'I'
                count += 1
