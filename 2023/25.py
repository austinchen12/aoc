...
network = nx.DiGraph()
network.add_nodes_from(keys)

for key in keys:
    for value in graph[key]:
        network.add_edge(key, value, capacity=1)
        network.add_edge(value, key, capacity=1)
result = None
for i in range(len(keys)):
    for j in range(i + 1, len(keys)):
        cut_value, partition = nx.minimum_cut(network, keys[i], keys[j])
        if cut_value == 3:
