...
for line in data.split('\n'):
    starts, ends = {}, {}
    for num in nums:
        result = line.find(num)
        if result != -1:
            starts[result] = num
        result = line.rfind(num)
        if result != -1:
            ends[result] = num
    ...
    letter_min = min(starts.keys()) if len(starts) else float('inf')
    letter_max = max(ends.keys()) if len(ends) else -1
    
    tens = (nums.index(starts[letter_min]) + 1) if letter_min < first or first == -1 else int(line[first])
    ones = (nums.index(ends[letter_max]) + 1) if letter_max > last or first == -1 else int(line[last])
    sum_ += 10 * tens + ones
print(sum_)
