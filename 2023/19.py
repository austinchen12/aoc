data = """vm{a>1183:zz,s>1553:A,s<827:xc,hls}
...
for workflow in workflows.split('\n'):
    start, end = workflow.index('{'), workflow.index('}')
    
    name = workflow[:start]
    content = workflow[start + 1:end].split(',')
    
    rules = []
    for rule in content[:-1]:
        trait, operator, constant, dest = regex.match(rule).groups()
        rules.append((trait, operator, int(constant), dest))
    workflow_rules[name] = (rules, content[-1])
...
for part in parts.split('\n'):
    traits = {}
    for trait in part[1:-1].split(','):
        name, value = trait.split('=')
        traits[name] = int(value)
    start = 'in'
    while start in workflow_rules:
        rules, end = workflow_rules[start]
        for rule in rules:
            trait, operator, constant, dest = rule
            if operator == '>':
                if traits[trait] > constant:
                    start = dest
                    break
            elif operator == '<':
                if traits[trait] < constant:
...
mp = 'xmas'
def find_ranges_not_in_b(starting_ranges, range_b):
    result = []

    for start, end in starting_ranges:
        # Check if the current range is completely outside of range B
        if end < range_b[0] or start > range_b[1]:
            result.append((start, end))
        else:
            # Adjust the range if it overlaps with range B
            if start < range_b[0]:
                result.append((start, range_b[0] - 1))
            if end > range_b[1]:
                result.append((range_b[1] + 1, end))

    return result
def find_ranges_in_b(starting_ranges, range_b):
    result = []

    for start, end in starting_ranges:
        # Check if the current range is completely within range B
        if start >= range_b[0] and end <= range_b[1]:
            result.append((start, end))
        else:
            # Adjust the range if it partially overlaps with range B
            if start < range_b[0] and end >= range_b[0]:
                result.append((range_b[0], min(end, range_b[1])))
            elif start <= range_b[1] and end > range_b[1]:
                result.append((max(start, range_b[0]), range_b[1]))

    return result
results = {'A': [], 'R': []}
def handle_rule(key, ranges):
    if key == 'A':
        results['A'].append(ranges)
        return
    elif key == 'R':
        results['R'].append(ranges)
        return
    
    assert key in workflow_rules
    rules, end = workflow_rules[key]
    for rule in rules:
        trait, operator, constant, dest = rule
        starting_ranges = ranges[mp.index(trait)]
        if operator == '<':
            new_range = (1, constant - 1)
        elif operator == '>':
            new_range = (constant + 1, 4000)
        else:
            assert False
        
        new_ranges = deepcopy(ranges)
        new_ranges[mp.index(trait)] = find_ranges_in_b(starting_ranges, new_range)
        handle_rule(dest, new_ranges)

        ranges[mp.index(trait)] = find_ranges_not_in_b(starting_ranges, new_range)
    else:
        handle_rule(end, ranges)
handle_rule('in', [[(1, 4000)] for _ in mp]) # [a, b]
sum_ = 0
for result in results['A']:
    prod = 1
    for trait in range(len(result)):
        for rng in result[trait]:
            prod *= rng[1] - rng[0] + 1
    sum_ += prod
print(sum_)